...
-- Create albums table for curated collections
...
-- Create album_items junction table for many-to-many relationship
...
DROP POLICY IF EXISTS "Admin users can create albums" ON public.albums;
DROP POLICY IF EXISTS "Admin users can update albums" ON public.albums;
DROP POLICY IF EXISTS "Admin users can delete albums" ON public.albums;
DROP POLICY IF EXISTS "Anyone can view album items" ON public.album_items;
DROP POLICY IF EXISTS "Admin users can add items to albums" ON public.album_items;
DROP POLICY IF EXISTS "Admin users can remove items from albums" ON public.album_items;
...
CREATE POLICY "Admin users can create albums"
...
-- Create RLS policies for album_items
CREATE POLICY "Anyone can view album items"
ON public.album_items FOR SELECT
TO public
USING (true);

CREATE POLICY "Admin users can add items to albums"
ON public.album_items FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = added_by);

CREATE POLICY "Admin users can remove items from albums"
...
CREATE INDEX IF NOT EXISTS idx_album_items_item_id ON public.album_items(item_id);
CREATE INDEX IF NOT EXISTS idx_album_items_added_by ON public.album_items(added_by);
...
WHERE table_schema = 'public' 
  AND table_name IN ('archive_likes', 'archive_comments', 'albums', 'album_items')
ORDER BY table_name;
...
SELECT 
  ...
  cmd
FROM pg_policies 
WHERE tablename IN ('archive_likes', 'archive_comments', 'albums', 'album_items')
